...
#       - multiple bounding box annotations per image.
#--------------------------------------------------------------------------------------------------------------------------
...
def main():
    ...
    xml_df.to_csv('bbox-data/spectrogram_annotations.csv', index=None) # writes the xml_df dataframe (just created) to a .csv file (that we can open with Excel).
...
#     column_name = ['filename', 'width', 'height', 'class', 'xmin', 'ymin', 'xmax', 'ymax'] # give the columns in the .csv file a name. 
#     xml_df = pd.DataFrame(xml_list, columns=column_name) # convert list of values into a structured pandas dataframe
#     return xml_df 


# def main():
#     datasets = ['training', 'validation'] # generate two .csv files for training and validation datasets. 

#     for ds in datasets: # first we do this for all the .xml files in the 'training' folder, then we do it for all the .xml files in the 'validation' folder. 
#         image_path = os.path.join(os.getcwd(), 'Images', ds) # build the path to the .xml files
#         xml_df = xml_to_csv(image_path, SKIP_NEGATIVES) # converting .xml files to .csv files
#         xml_df.to_csv('Data/{}_data.csv'.format(ds), index=None) # writes the xml_df dataframe (just created) to a .csv file (that we can open with Excel).
