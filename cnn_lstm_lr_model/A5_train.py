...
np.random.seed(42)
working_folder = "/home/wairimu/training_data_moving_avg_signal_power"
...
def prepare_data():
    ...
    # lydia's calling...
    # features = data['cnn_input'].astype(np.float32) # 'cnn_input' is 2D (num_files, num_windows) because MATLAB drops 1's entries when saving tensors as .mat files. 
    # print("shape of 'cnn_input'", features.shape)
    # labels = data['labels_vector'].flatten().astype(np.float32)
    # labels = labels[:-4]
    ...
    # reshape features to 4D
    features = features[:, :, :, np.newaxis]
    print("shape of reshaped 'cnn_input'", features.shape)
    ...
    with open(working_folder+"/test_set.pkl", "wb") as f:
...
# plt.show()
plt.savefig(working_folder+"/train_plot.pdf")