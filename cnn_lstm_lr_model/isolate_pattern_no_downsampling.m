function [has_pattern, moving_avg_power_vec, start_sec_first_pattern_chunk, end_sec_last_pattern_chunk] = isolate_pattern_no_downsampling(iq_signal_file, pattern_length, to_downsample)

% -------------------------------------------------------------------------------------------------------
% function: isolate_pattern().
%
% purpose: automate the process of annotating start/end times the chunk of the file that captures the pattern -- all characteristic spikes.
%  
% note: - if a signal file has at least 2 consecutive peaks, it gets labeled 'has_pattern=1'
%       - if a signal file has only isolated peaks, it gets labeled 'has_pattern=0'. 
%
% inputs: - the .raw IQ signal file
%
% outputs: - has_pattern boolean variable (a file-level label to classify whether or not a signal file has our target pattern or not).
...
if to_downsample ~= -1
    ...
    disp(to_downsample);
    iq_signal_read = downsample(iq_signal_read, to_downsample);
end
...
change_locs = findchangepts(y_downsampled, 'Statistic', 'linear', 'MinDistance', min_distance, 'MinThreshold', 0.2); % option to specify MaxNumChanges.
...
for j = 1:num_change_points
    ...
    for k = 1:num_peak_points
        ...
        % if the change point that comes right after the k-th peak point is < 1 seconds, find the next change point.
        if next_change_point - peak_point < 0.5
