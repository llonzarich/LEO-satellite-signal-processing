...
with open("test_set.pkl", "rb") as f: # TODO: comment/uncomment before running script.
    X_test, y_test, testing_files = pickle.load(f)
...
def display_results(y_test, predictions, class_names):
    ...
    # btw: y_test is originally shape (num_files)
    # btw: 'predictions' (the output of the model) is shape (num_files, 1)
    
    y_test = y_test
    y_scores = (predictions >= 0.5).astype(int).flatten() # convert probability predictions into binary predictions based on threshold.
