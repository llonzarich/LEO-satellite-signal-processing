...
class CNN_LSTM_Classifier():
    ...
    def _build_model(self, output_bias=None):
        inputs = Input(shape=(self.num_windows, self.num_samples_per_window, self.num_channels))

        # model = Sequential([
        #                     Dense(16, 
        #                           activation='relu',
        #                           input_shape=(self.num_windows, self.num_samples_per_window, self.num_channels)
        #                           ),
        #                     Dropout(0.5),
        #                     Dense(1, activation='sigmoid',
        #                         bias_initializer=output_bias),
        #                     ])
        x = Dense(16, activation='relu', input_shape=(self.num_windows, self.num_samples_per_window, self.num_channels)) (inputs)
        x = Dropout(0.5)(x)
        outputs = Dense(1, activation='sigmoid')(x)
        model = Model(inputs = inputs, outputs = outputs)
        print("-----------------------------------------------------------------")
...
def prepare_data():
    ...
    # # file_names = data['combined_file_list']    
    ...
    # import pandas as pd
    # df = pd.DataFrame([y_test, testing_files])
    # df.to_csv("/home/wairimu/training_data_stricter_classification_method/test_data_plots/testing_data.csv")
...
# plt.show()
plt.savefig("/home/wairimu/train.png")
