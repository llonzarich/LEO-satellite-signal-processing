...
% function preparing_cnn_matrix_1D_fn(path, tensor_fname, start_ix)
    ...
    for i = start_ix:end_ix
        ...
        [~, has_pattern, ~, start_sec_first_pattern_chunk, end_sec_last_pattern_chunk] = thirtysixsec_isolate_pattern(filepath);
        ...
        trimmed_file = trim_file(filepath, start_sec_first_pattern_chunk, end_sec_last_pattern_chunk, F_s);


        % split the signal into 35 2sec windows w/ 1sec stride. 
        [~, num_windows, ~, iq_components] = thirtysixsec_window_signal(trimmed_file);
