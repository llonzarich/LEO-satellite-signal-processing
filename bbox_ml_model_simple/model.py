...
class SignalDetection:
    def __init__(self, input_shape=(224,224,(3,3))): # 3 is the most commonly used kernel size.
        # self.classifier = load_model(classifier_model_path)
        # self.localizaer = load_model(localizer_model_path)
        self.input_shape = input_shape
    

    def load_data(self, annotations_path):
        print("Loading csv spectrogram annotations dataset from: ", annotations_path)
        rows = open(annotations_path).read().strip().split("\n")[1:] # read content of csv file into a string, removes trailing or leading whitespaces, splits string separate elements in a list, each on a 'row'.
        ...
        for row in rows: 
            ...
            # normalize the bbox coordinates to the range [0,1].
            ...
            image = load_img(image_path, target_size=self.input_shape[:2])) # ensure image is 224x224 pixels for training
            ...
            labels.append(1) # added 6/8. Could be 0 if no signal later
        ...
        return train_test_split(images, targets, labels, test_size=0.1, randome_state=42)
    ...
    def build_model(self):
        ...
        self.model = Model(inputs=input_layer, outputs=[classification_branch, locator_branch]) #add 6/8
        ...
        self.model.compile(loss=losses, optimizer=keras.optimizers.Adam(learning_rate=0.0001), metrics=['accuracy', 'accuracy'])
        
        self.model.summary() 

    
    def train(self, train_images, train_targets, train_labels, val_images, val_targets, val_labels):
        ...
        train_targets_dict = { # tells keras what target goes to what output
            "cb_head": train_labels, # the first node of the cb is assigned to. no. output for classifing.
        ...
        self.history = self.model.fit(train_images, train_targets_dict, validation_data=(val_images, val_targets_dict), batch_size=4, epochs=config.num_epochs, shuffle=True, verbose=1)
    ...
    def evaluate(self, te)


    def save_model(self, path):

        self.model.save("output/detector.h5")
...
# model.compile(loss=losses, optimizer=keras.optimizers.Adam(lr=0.0001), metrics=['accuracy', 'accuracy'])
# print(model.summary())
# # print("HELLLLOOOO")
